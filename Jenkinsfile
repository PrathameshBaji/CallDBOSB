...
def desc = "describe --abbrev=0 --tags"
def ls = "ls-remote --tags origin"
def lsout
def descout
...
pipeline {
   ...
   stages {
      stage('Build') {
        steps {
          echo "Hi Prathamesh Test PracticeCICD..."
          echo "${myvar1}"
          echo "${myvar2}"
	   ...
	   //   checkout([$class: 'GitSCM', branches: [[name: '*/master']], doGenerateSubmoduleConfigurations: false,  extensions: [[$class: 'CleanCheckout']], submoduleCfg: [], userRemoteConfigs: [[url: 'https://github.com/PrathameshBaji/PracticeCICDNew.git']]])
	     ...
	     // New SCM details	   
	      ...
	      //Git Envionment Vars
              ...
              echo "Git Tags : ${env.GIT_TAG_NAME}"
	      echo "Git Tags : ${env.GIT_COMMIT}"
	      echo "Git Tags : ${env.GIT_COMMITER_NAME}"
	      echo "Git Tags : ${env.GIT_URL_N}"
	      echo "Git Tags : ${env.GIT_PREVIOUS_COMMIT}"
	      echo "Git Tags : ${env.GIT_PREVIOUS_SUCCESSFUL_COMMIT}"
		...
		//Script to get the Tags from Remote GIT Repository
		script{
	               env.GIT_TAG_NAME = bat (returnStdout: true,script:'git describe --abbrev=0 --tags').trim()
			env.GIT_TAG_NAME = env.GIT_TAG_NAME.tokenize('\n').last()
			echo "${env.GIT_TAG_NAME}"
		       env.GIT_TAG_NAME = bat(returnStdout: true, script: 'git ls-remote --tags origin').trim()
			env.GIT_TAG_NAME = env.GIT_TAG_NAME.tokenize('\n').last()
			echo "${env.GIT_TAG_NAME}"
		       env.GIT_TAG_NAME = bat(returnStdout: true, script: 'git ls-remote --tags  https://github.com/PrathameshBaji/PracticeCICD.git').trim()
			env.GIT_TAG_NAME = env.GIT_TAG_NAME.tokenize('\n').last()
			echo "${env.GIT_TAG_NAME}"
           ...
           script{
		   // Get the changeSets from Current Build
